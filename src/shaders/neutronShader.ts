...
// 中性子用の頂点シェーダー
export const neutronVertexShader = `
...
// 中性子用のフラグメントシェーダー（黄緑色）
export const neutronFragmentShader = `
  uniform float time;        // 時間
  uniform vec3 color1;       // 明るい色（明るい黄緑）
  uniform vec3 color2;       // 中間の色（黄緑）
  uniform vec3 color3;       // 暗い色（暗い黄緑）
  varying vec3 vNormal;      // 法線ベクトル
  varying vec3 vPosition;    // 頂点位置
  varying vec2 vUv;          // UV座標
